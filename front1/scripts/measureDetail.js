...
const  fetchFFTData= async (id) => {
    try {
        // URL de tu backend que calcula la FFT
         const response = await fetch(`http://localhost:8080/evaluation/${id}/fft`, { 
            method: 'GET',
       ...
       // const data = await response.json(); // Datos del backend (frecuencia y espectro)
...
async function fetchAutocorrelation(id) {
    const response = await fetch(`http://localhost:8080/evaluation/${id}/autocorrelation`);
    const data = await response.json();
    return { lags: data.lags, autocorrNorm: data.autocorrNorm };  // Regresamos ambos campos
}
...
async function plotAutocorrelation(id) {
    ...
    new Chart(autoChart, {
        ...
        data: {
            labels: lags,  // Usar los lags en segundos como etiquetas para el eje X
            datasets: [{
